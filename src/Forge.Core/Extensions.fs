...
type XNode with
    ...
    static member ancestors (xnode:XNode) = xnode.Ancestors ()
    static member elementsBefore (xnode:XNode) = xnode.ElementsBeforeSelf () 
...
type XContainer with
    ...
    static member nodes (xcon:XContainer) =  xcon.Nodes () : seq<XNode>
...
let xelem name (content:seq<'a>)  = XElement (XName.Get name, Seq.toArray content)
let xattr name value    = XAttribute (XName.Get name, value)

let addElement (child:XElement) (parent:XElement) =
    parent.Add child
    parent

let setAttribute (attr:string) value (xelem:XElement) =
    xelem.SetAttributeValue (attr,value)
    xelem
